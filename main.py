...
def force_clean_data(df):
    ...
    # Omdøbning for at undgå dubletter
    rename_map = {
    ...
    def set_land(r):
        ...
        if any(x in t for x in ["denmark", "dk", "dansk", "partner-ads"]): return "🇩🇰"
    ...
    # WEBSITE GENERATOR (Sikrer links)
    def set_site(r):
...
# --- 6. POP-UP (DET KOMPLETTE KLIENT KORT) ---
...
def client_popup(idx):
    ...
    with t2:
        ...
        upd['Noter'] = st.text_area("Skriv logbog her...", value=row.get('Noter'), height=250)
        st.divider()
        st.file_uploader("Upload dokumentation (PDF, lyd, billede)", key=f"f_{idx}")
...
if not st.session_state.df.empty:
    ...
    st.info("💡 Klik på den grå boks helt til venstre for at åbne kortet.")
