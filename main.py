...
def force_clean_data(df):
    ...
    df = df.loc[:, ~df.columns.duplicated()] # Slet dublet-kolonner
...
# --- 6. POP-UP (ALLE 26 FELTER MAPPED) ---
...
def client_card(idx):
    ...
    with t1:
        ...
        with c2:
            ...
            upd['Opflg. dato'] = st.date_input("Næste opfølgning", value=safe_d(row.get('Opflg. dato'))).strftime('%d/%m/%Y')
            upd['Kontakt dato'] = st.date_input("Kontakt dato", value=safe_d(row.get('Kontakt dato'))).strftime('%d/%m/%Y')
    ...
    with t2:
        ...
        st.file_uploader("Upload fil til klient", key=f"f_{idx}")
...
with st.sidebar:
    ...
    st.divider()
    st.header("📥 Import")
    k_up = st.text_input("Kategori ved upload:", "Bolig, Have og Interiør")
